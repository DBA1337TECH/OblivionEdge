// Copyright © 1337_TECH
// Full-frame UDP packet capture module for Wireshark-compatible PCAP output.
...
#define CLASS_NAME  "suspect_udp"

#define RING_SIZE     4096
#define MAX_PKT_SIZE  4096    /* Jumbo-safe */

static struct nf_hook_ops nfho;

/* ABI struct — must match userspace expectations */
struct packet_entry {
...
/* Write ABI frame: [u32 len][packet bytes] */
static inline void ring_push(const u8 *buf, u32 len)
{
    if (ring_is_full()) {
        printk(KERN_WARNING "[suspect_udp_kmod] Ring full, dropping UDP packet\n");
        return;
    }
                              ...
                              const struct nf_hook_state *state)
{
    ...
    /* Safe full-frame extraction (handles nonlinear SKBs) */
    if (skb_copy_bits(skb, 0, tmp, pkt_len) < 0) {
        printk(KERN_WARNING "[suspect_udp_kmod] skb_copy_bits failed\n");
        kfree(tmp);
        return NF_ACCEPT;
    }
...
static int __init suspect_init(void)
{
    ...
    /* PRE_ROUTING ensures full frames before NAT/conntrack */
    nfho.hook     = hook_func;
    ...
    printk(KERN_INFO "[suspect_udp_kmod] UDP capture module loaded.\n");
    return 0;
}
...
static void __exit suspect_exit(void)
{
    ...
    printk(KERN_INFO "[suspect_udp_kmod] Unloaded.\n");
}
...
MODULE_DESCRIPTION("Full-frame UDP capture module for Oblivion Edge");
